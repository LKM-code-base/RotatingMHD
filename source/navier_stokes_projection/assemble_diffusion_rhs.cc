 ...
 VelocityRightHandSideAssembly::MappingData<dim>       &data)
{
  ...
  if (body_force_ptr != nullptr)
    body_force_ptr->vector_value_list(scratch.velocity_fe_values.get_quadrature_points(),
                                      scratch.body_force_values);
  else
    scratch.body_force_values = std::vector<Vector<double>>(scratch.n_q_points,
                                                            Vector<double>(dim));
