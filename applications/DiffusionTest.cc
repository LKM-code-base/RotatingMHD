...
void Diffusion<dim>::assemble_system_rhs()
{
  ...
  for (const auto &cell: dof_handler.active_cell_iterators())
  {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        explicit_term[q] = alpha[1] * old_values[q];
        diffusion_term[q] = gamma[1] * time_step / peclet * old_gradients[q];
...
template <int dim>
void Diffusion<dim>::output_results() const
{
  DataOut<dim> data_out;
  data_out.attach_dof_handler(dof_handler);
  data_out.add_data_vector(solution, "U");
  data_out.build_patches();
  data_out.set_flags(DataOutBase::VtkFlags(discrete_time.get_current_time(),
                                           discrete_time.get_step_number()));
  const std::string filename =
    "solution-" + Utilities::int_to_string(discrete_time.get_step_number(), 3) + ".vtk";
...
void Diffusion<dim>::run()
{
  ...
  {
    double time_step = 0.1 * std::pow(0.5, cycle);

    std::cout << "Solving until t = "
    ...
    {
      std::cout << discrete_time << std::fixed << std::endl;
      ...
      if ((discrete_time.get_step_number() % 100000 == 0) ||
          (discrete_time.get_current_time() == discrete_time.get_end_time()))
        output_results();
    }
...
int main()
{
  try
  {
    ...
    parameters.maximum_time_step = 0.1;
