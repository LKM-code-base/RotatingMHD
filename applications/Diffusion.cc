...
scalar_field(std::make_shared<Entities::ScalarEntity<dim>>(
  ...
  this->triangulation,
  "Scalar field")),
exact_solution(std::make_shared<ExactSolution<dim>>(
  parameters.Pe,
...
template <int dim>
void Diffusion<dim>::postprocessing()
{
...
template <int dim>
void Diffusion<dim>::solve(const unsigned int level)
{
  setup_dofs();
  setup_constraints();
  scalar_field->reinit();
...
void Diffusion<dim>::run()
{
  ...
  *this->pcout << convergence_table;
  ...
  tablefilename << ((parameters.convergence_test_parameters.test_type ==
  									 ConvergenceTest::ConvergenceTestType::spatial)
                     ? "Diffusion_SpatialTest"
                     : ("Diffusion_TemporalTest_Level" + std::to_string(parameters.spatial_discretization_parameters.n_initial_global_refinements)))
                << "_Pe"
                << parameters.Pe;
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    RunTimeParameters::ProblemParameters parameter_set("Diffusion.prm", true);
