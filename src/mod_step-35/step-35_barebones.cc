...
namespace Step35
{
  ...
  void NavierStokesProjection<dim>::setup_initialize_matrices_and_vectors()
  {
      ...
      Table<2,DoFTools::Coupling> coupling(dim,dim);
        for (unsigned int c=0; c<dim; ++c)
          for (unsigned int d=0; d<dim; ++d)
            if (c == d)
              coupling[c][d] = DoFTools::Coupling::always;
            else
              coupling[c][d] = DoFTools::Coupling::none;
      DoFTools::make_sparsity_pattern(v_dof_handler,
                                      coupling,
                                      dsp);
      v_sparsity_pattern.copy_from(dsp);
      std::ofstream out ("results_barebones/v_sparsity_pattern.gpl");
...

    {
      ...
      std::ofstream out ("results_barebones/p_sparsity_pattern.gpl");
...

    {
      ...
      std::ofstream out ("results_barebones/mixed_sparsity_pattern.gpl");
  ...
  void NavierStokesProjection<dim>::data_output(const unsigned int step)
  {
    ...
    std::ofstream       output_file("results_barebones/solution-" 
